...
//	double accelerationZ;
	...
	public void pulseLeft(){
		Robot.drivetraintank.setLeft_Back(.2);
    	Robot.drivetraintank.setLeft(.2);
	}
	public void pulseRight(){
		Robot.drivetraintank.setRight(.2);
    	Robot.drivetraintank.setRight_Back(.2);
	}
//	public void getData(){
    ...
    protected void execute() {
 		...
 		if(gearSpoke == false){
	 		...
	 		if(rangeLeft > rangeRight){
	 			double diff = rangeLeft - rangeRight;
	 			while(diff > 3.0){
	 				rangeLeft = ultraLeft.getRangeInches();
		        	rangeRight = ultraRight.getRangeInches();
		        	diff = rangeLeft - rangeRight;
	 				pulseRight();
	 			}
	 		} else {
	 			double diff = rangeRight - rangeLeft;
	 			while(diff > 3.0){
	 				rangeLeft = ultraLeft.getRangeInches();
		        	rangeRight = ultraRight.getRangeInches();
		        	diff = rangeRight - rangeLeft;
	 				pulseLeft();
	 			}
	 		}
	 		isAligned = true;
...
// 			double diff = Math.abs(rangeLeft - rangeRight);
// 			if(rangeLeft > rangeRight && diff < 3.0){
// 				while(diff > 3.0){
// 					pulseRight();
// 				}
// 				while(diff < 2.0){
